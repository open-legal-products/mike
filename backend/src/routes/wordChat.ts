...
// Express 4 drops a rejected async handler on the floor: no response, no log,
// the socket held until the client gives up. asyncRoute forwards the rejection
// to next() so app.ts's terminal handler can answer.
  ...
  asyncRoute(async (req, res) => {
    ...
    if (assistantMessageIds.length > 0) {
      ...
      if (editsError) {
        ...
        return void res
          .status(500)
          .json({ detail: "Failed to load Word chat" });
      }
...
wordChatRouter.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    try {
      ...
      if (saveError) {
        console.error(
          "[word-chat] failed to save assistant response",
          saveError,
        );
        write(
          `data: ${JSON.stringify({
      ...
      if (isAbortError(error)) {
        ...
        if (error instanceof AssistantStreamError) {
          ...
          if (saveError) {
            console.error(
              "[word-chat] failed to save aborted stream",
              saveError,
            );
          }
        }
      ...
      try {
        ...
        const normalizedErrorEvents =
          await normalizeAssistantEvents(errorEvents);
        const saveError = await updateAssistantMessage(
