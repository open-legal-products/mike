...
// collision — happens inside one RPC. The route validates the segment list
// before calling; this owns access, the base-folder check, and the RPC.
...
export async function resolveProjectFolderPath(
  ...
  args: {
    ...
    segments: string[];
    conflictResolution: "reuse" | "rename" | "error";
  },
): Promise<ResolveFolderPathResult> {
  const { projectId, userId, userEmail, baseFolderId, segments } = args;
