...
// Ownership check for a folder id that arrived on a request the service
// doesn't otherwise own (the upload endpoint hands the row itself to the
// documents module), kept here so route handlers never query directly.
export async function ensureLibraryFolderExists(
  db: Db,
  userId: string,
  kind: LibraryKind,
  folderId: string,
