...
-- Contract-review playbooks: a mutable draft per playbook, immutable numbered
-- published versions, review runs against a published version, and the audit
-- trail for Word imports. Reviews read the published version, so editing a
-- draft never changes an in-progress or historical review.
