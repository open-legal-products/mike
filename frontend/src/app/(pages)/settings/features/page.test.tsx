...
describe("FeaturesPage Legal Data Hunter", () => {
    ...
    it("disables every existing LDH connector without deleting any", async () => {
        ...
        const disabledConnector = { ...connectedConnector, enabled: false };
        const disabledDuplicateConnector = {
            ...duplicateConnector,
            enabled: false,
        };
        mocks.listMcpConnectors
            .mockResolvedValueOnce([connectedConnector, duplicateConnector])
            .mockResolvedValueOnce([connectedConnector, duplicateConnector])
            .mockResolvedValueOnce([
                disabledConnector,
                disabledDuplicateConnector,
            ]);
        render(<FeaturesPage />);
    ...
    it("reuses an existing LDH connector instead of creating a duplicate", async () => {
        const user = userEvent.setup();
        mocks.listMcpConnectors
            .mockResolvedValueOnce([connector])
            .mockResolvedValueOnce([connector])
        ...
        await waitFor(() =>
            expect(mocks.refreshMcpConnectorTools).toHaveBeenCalledWith(
                connector.id,
            ),
        );
        expect(mocks.createMcpConnector).not.toHaveBeenCalled();
    ...
    it("rolls an enabled connector back off when OAuth startup fails", async () => {
        const user = userEvent.setup();
        const createdConnector = { ...connector, enabled: true };
        mocks.listMcpConnectors
            .mockResolvedValueOnce([])
            .mockResolvedValueOnce([])
            .mockResolvedValueOnce([createdConnector])
            .mockResolvedValueOnce([]);
        mocks.createMcpConnector.mockResolvedValue(createdConnector);
    ...
    it("completes required MFA before creating the connector", async () => {
        ...
        await waitFor(() => expect(mocks.createMcpConnector).toHaveBeenCalled());
    });
});
