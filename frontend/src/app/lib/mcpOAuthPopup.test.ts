...
describe("authorizeMcpConnector", () => {
    ...
    it("keeps the overall timeout while checking a closed popup", async () => {
        vi.useFakeTimers();
        const popup = popupWindow();
        vi.spyOn(window, "open").mockReturnValue(popup);
        startMcpConnectorOAuth.mockResolvedValue({
        ...
        getMcpConnector.mockReturnValue(new Promise(() => {}));

        const authorization = authorizeMcpConnector("connector-1");
        const result = expect(authorization).rejects.toThrow(
        ...
        await result;
        expect(getMcpConnector).toHaveBeenCalledOnce();
        vi.useRealTimers();
    });
