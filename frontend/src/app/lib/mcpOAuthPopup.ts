...
export function openMcpOAuthPopup(connectorId = "pending"): Window | null {
    return window.open(
        "about:blank",
...
export async function authorizeMcpConnector(
    connectorId: string,
    popup: Window | null = openMcpOAuthPopup(connectorId),
    ...
    try {
        ...
        await new Promise<void>((resolve, reject) => {
            ...
            const poll = window.setInterval(() => {
                if (!popup.closed || checkingClosedPopup) return;
