 ...
 * so callers can recognise codes like `mfa_verification_required`. No headers
 * are set at all: authentication rides the HttpOnly cookie apiFetch sends, and
 * an empty headers object would still be an explicit (boundary-less) override.
 */
...
export async function uploadProjectDocument(
    ...
    form.append("file", file);
    if (folderId) form.append("folder_id", folderId);
    return apiUploadRequest<Document>(`/projects/${projectId}/documents`, form);
}
