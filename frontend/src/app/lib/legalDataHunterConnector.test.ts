...
describe("Legal Data Hunter connector identity", () => {
    it("matches equivalent spellings of only the built-in LDH endpoint", () => {
        ...
        expect(
            isLegalDataHunterConnector(
                connector("https://LEGALDATAHUNTER.com:443/mcp"),
            ),
        ).toBe(true);
        expect(
            isLegalDataHunterConnector(
                connector("https://legaldatahunter.com/mcp?tenant=other"),
            ),
        ).toBe(false);
        expect(
